...
class VariableCalculation:
    ...
    def get_ewm_volume(self,V, dur_l=60, dur_s=20):
        """
        Parameters
        ----------
        ...
        """
        ewm_vlm_l = self.get_ewm_time_series(P=V, alpha=1-1/dur_l)  # 'X' 대신 'P'를 사용
        ewm_vlm_s = self.get_ewm_time_series(P=V, alpha=1-1/dur_s)  # 'X' 대신 'P'를 사용
