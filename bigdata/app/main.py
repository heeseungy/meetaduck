...
def update_result_wordcount(json_data, favorability, guest_id : int, isManito:bool) :
    connection = get_db_connection()
    try: 
        with connection.cursor() as cursor:
...
async def word_count_spark(guest_id: int):   
    ...
    print(" \n me_maniti_favorability ", me_maniti_favorability)
    if pd.isna(me_maniti_favorability):
        me_maniti_favorability = 0

    me_maniti_wordcount = word_count(guest_id,manito_info["chat_id"] )
    update_result_wordcount(me_maniti_wordcount, me_maniti_favorability, guest_id, False)
